...
video_file = VideoWriter('project.avi');
open(video_file);
for i=0:8:4000
    ...
    line([350 200],[-100 -100],'Color','w','LineWidth',5);
    %-----------------회전 변환된 차 그리기----------------
    patch(mycar_data_rot,'FaceColor','none','EdgeColor','none'); % 검은색
    ...
    if i<=1200 % 검은차가 사거리 직전까지 직진하는 구간
        axis([-400 600 -1800+i -800+i 0 50]);
    elseif i>1200 && i<2720 % 검은차가 신호를 기다리는 구간
        axis([-400 600 -600 400 0 50]);
        view(0,90);
    elseif i>=2720 % 검은차가 좌회전을 하는 구간
        if i<=3080
            view((i-2720)/4,90); % 360을 갈 동안 0~90도로 뷰포인트를 바꿔야 하기 때문에
            axis([-1000-1.5*(i-2720) 500 -300 500 0 50]);
        end
        if i>3080 % 좌회전 끝내고 직진하는 구간
            view(90,90);
            axis([-1500-(i-3100) 200-(i-3100) -300 500 0 50]);
        end
    end
    campos(mycar_data_rot.vertices(439,(1:3))+[0 0 30+15.3]);
    camtarget(mycar_data_rot.vertices(211,(1:3)) + [0 0 30]);
    %% --------------------------------------------------
    ...
    %------------------그래프 설정---------------
    if i<=1200 % 검은차가 사거리 직전까지 직진하는 구간
        axis([-200 300 -1800+i -600+i 0 150]);
        view(200,20);
    elseif i>1200 && i<2920 % 검은차가 신호를 기다리는 구간
        axis([-200 300 -600 600 0 150]);
        view(200,20);
    elseif i>=2920 % 검은차가 좌회전을 하는 구간
        axis([-100-(i-2920) 300 -600 600 0 150]);
        if i<=3280
            view((i-2920)/4+200,20); % 180을 갈 동안 200~290도로 뷰포인트를 바꿔야 하기 때문에
            axis([-500 400 -300 500 0 150]);
        end
        if i>3280 % 좌회전 끝내고 직진하는 구간
            view(290,20);
            axis([-500-(i-3280) 400-(i-3280) -300 500 0 150]);
        end
    end
    pause(0.000001);hold on;
